...
class HoverNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out_nc = self.decoder_nc(x)

        out_np = self.head_np(out_np)
        # out_hv = self.head_hv(out_hv)
        out_nc = self.head_nc(out_nc)

        # assert out_np.shape[0] == out_hv.shape[0] == out_nc.shape[0] == inputs.shape[0]
        # print(out_np.shape, out_hv.shape, out_nc.shape)
        # assert out_np.shape[2] == out_hv.shape[2] == out_nc.shape[2] == inputs.shape[2]
        # assert out_np.shape[3] == out_hv.shape[3] == out_nc.shape[3] == inputs.shape[3]
