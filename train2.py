...
log_path = '/home/jqshen/MyCode/MyModel/hover2.log'
metric_store_path = '/home/jqshen/MyCode/MyModel/hover2.npy'
C_LOGGING = {
    'version': 1,
...
class Trainer(object):
    ...
    def __init__(self, model, batch_size: int, max_epoch: int, lr: float,
        ...
        self.store = {'train': {'loss': [], 'loss_np': [], 'loss_nc': [], 'loss_hv': []},
                      'val': {'loss': [], 'loss_np': [], 'loss_nc': [], 'loss_hv': []}}
        self.epoch_loss = {'loss': [], 'loss_np': [], 'loss_nc': [], 'loss_hv': []}
...
if __name__ == "__main__":
    model_trainer = Trainer(model=model2.HoverNet(), batch_size=4, max_epoch=50, lr=1e-4,
    ...
    def metric_plot(phase, scores):
        ...
        plt.plot(range(len(scores["loss_nc"])),
                 scores["loss_nc"],
                 label='loss_nc')
        plt.plot(range(len(scores["loss_hv"])),
                 scores["loss_hv"],
                 label='loss_hv')
        plt.title(f'{phase}')
        plt.xlabel('Epoch')
    ...
    tester=test.predict(datafolder=data_dir,model_save_path=model_save_path)
    tester.start()
'''
model_trainer.meter
